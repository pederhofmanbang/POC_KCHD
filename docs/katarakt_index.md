...
**Senast uppdaterad:** 2026-03-28 (v2.4)
...
| **—** | **katarakt_vantetid_v2.4.vql** | **Komplett v2.4: 17 vyer, 3 nya (ref_avvikelsekoder, ref_filter_region, per-dim utökade), fixad patientresa, sjukhus i verif/DQ6, is_hemregion** | **✅ Klar, testad** | **Ej skickat** |
| 1a | verifieringspaket.vql | verif_jamforelse + verif_stickprov (2 vyer) | ✅ Klar | Ja (2026-03-26) — ersätts av v2.4 |
| 1b | kvalitetspaket.vql | dq0–dq6 + dq_rapport_data (8 vyer, 373 rader) | ✅ Klar | Ja (2026-03-26) — ersätts av v2.4 |
| 1b | dq_rapport.html | Lokal DQ-rapport, paste-to-render, multi-separator | ✅ Klar | Ja (2026-03-26) |
| 2 | fhir_paket.vql | ref_fhir_system + fhir_measure_report v3.1 (2 vyer, alla 16 KPI:er × alla dimensioner, 30 SELECT-block) | ⚠️ 3 fixar behövs | Ej skickat — levereras med fas 3 |
| 3 | KchdFhirSerializer/ (C#) | .NET 8 + Hl7.Fhir.R4 (Firely SDK). Läser TSV/ODBC, producerar FHIR Bundle JSON | ✅ Verifierad | Ej skickat — levereras med fas 2 |
| 3 | fhir_serializer.py | Python-referensimplementation — läser TSV, producerar FHIR Bundle JSON | ✅ Klar | I repo (intern referens) |
| 3 | fhir_bundle_example.json | Referens-JSON: 12 MeasureReports, 214 KB, validerad | ✅ Klar | I repo (intern referens) |
| 4 | fhir_radniva.vql + C# | Procedure/ServiceRequest + SSIS | 📋 Ej påbörjat | — |

**Fas 2+3 levereras som ett paket.** Fas 2 (VQL) utan fas 3 (C#) ger VGR en tabell de inte kan göra något med. Hela kedjan — VQL → C#-serialiserare → FHIR JSON — levereras när vi vet att VGR:s miljö stödjer .NET 8 (eller vi publicerar som self-contained .exe).

**v2.4 ersätter alla tidigare VQL-filer.** katarakt_vantetid_v2.4.vql innehåller alla 17 vyer (fas 0, 1a, 1b) i en fil. De separata filerna (komplettering_detaljvyer.vql, verifieringspaket.vql, kvalitetspaket.vql) behövs inte längre.
...
| test_csharp.sh | Bygger + kör C#-serialiseraren, jämför mot Python-referens | `bash test_csharp.sh` |
...
| D19 | Fas 2+3 levereras som ett paket, aldrig separat | VQL utan C# ger VGR en tabell de inte kan göra något med. Hela kedjan levereras samlat. | 2026-03-28 |
| D20 | C# verifierad i GitHub Codespace (.NET 8) | dotnet build OK, 440 rader → 12 MeasureReports, semantisk match mot Python-referens | 2026-03-28 |
| D21 | v2.4: ref_filter_region för utomläningsflagga | Konfigurerbar länskod, is_hemregion-flagga i calc — filtrerar INTE, bara flaggar. Kod behöver inte ändras oavsett om utomläningar förklarar avvikelsen eller ej. | 2026-03-28 |
| D22 | v2.4: ref_avvikelsekoder dokumenterar koderna | Referensvy som ref_kva_katarakt. XSD-format (1/2/3), ⚠️-kommentarer i alla vyer om textformat. | 2026-03-28 |
| D23 | v2.4: per-dim-vyer utökade med alla KPI:er | Alla 5 per-dim-vyer har nu samma kolumner som res_kpi_manad (andel, medel, patientresa, sjukhusinfo). Krävdes för att FHIR-paketet ska producera alla 5 KPI:er per dimension. | 2026-03-28 |
| D24 | v2.4: sjukhusnamn i verif_jamforelse + DQ6 | VGR:s feedback — behövs för att jämföra per sjukhus mot vantetider.se. | 2026-03-28 |
| D25 | PvV exkluderas för genomförda, MoV inkluderas — bekräftat mot SKR-dokumentation | SKR:s dokumentation: "MoV kan exkluderas vid redovisning av väntande" men nämner inte exkludering för genomförda. Vår kod korrekt. | 2026-03-28 |
...
| U7 | Fas 3: .NET 8 runtime + SSIS-version hos VGR | Mail redo att skicka. Om .NET 8 saknas → publicera som self-contained .exe |
| U8 | Fas 4: FHIR Procedure/ServiceRequest — exakt mappning | Väntar på fas 2+3 i produktion |
| ~~U9~~ | ~~Per-dimensioner för median/medel/patientresa~~ | ✅ Löst — alla 5 mått per dimension byggt i v3 |
| U10 | fhir_paket.vql v3.1: applicera 3 fixar | Se FHIR_PAKET_FIXAR.md — kolumnnamn i VQL, TSV-alias oförändrade |
| U11 | VGR utomläningstest: AND LEFT(lkf,2) = '14' | Väntar på VGR:s svar — förklarar troligen avvikelse mot vantetider.se |
| U12 | Pusha v2.4 + fixat fhir_paket.vql till GitHub | Allt klart, väntar på paketering |
...
## 10. VYLISTA I DENODO (17 vyer i v2.4 + 2 FHIR-vyer)

| Lager | Vy | Fas | v2.4 ändring | Status |
|-------|-----|-----|-------------|--------|
| 00 | ref_kva_katarakt | — | Oförändrad | ✅ Live hos VGR |
| 00 | **ref_avvikelsekoder** | — | **NY** — dokumenterar kod 1/2/3 | ✅ Klar |
| 00 | **ref_filter_region** | — | **NY** — konfigurerbar länskod per region | ✅ Klar |
| 01 | src_genomford | — | Oförändrad (mall) | ✅ Live |
| 01 | src_vantande | — | Oförändrad (mall) | ✅ Live |
| 02 | calc_vantetid_genomford | — | **Ändrad** — +is_hemregion, fixad patientresa-kommentar | ✅ Klar, testad |
| 02 | calc_vantande | — | Oförändrad | ✅ Live |
| 03 | res_kpi_manad | — | **Ändrad** — +sjukhusnamn, +hsaid_region i GROUP BY | ✅ Klar, testad |
| 03 | res_kpi_per_yrke | — | **Utökad** — alla KPI:er + sjukhusinfo | ✅ Klar, testad |
| 03 | res_kpi_per_kontaktform | — | **Utökad** | ✅ Klar, testad |
| 03 | res_kpi_per_bokning | — | **Utökad** | ✅ Klar, testad |
| 03 | res_kpi_per_remittent | — | **Utökad** | ✅ Klar, testad |
| 03 | res_kpi_per_kommun | — | **Utökad** | ✅ Klar, testad |
| 03 | res_genomford_detalj | — | **Ändrad** — +is_hemregion | ✅ Klar |
| 03 | res_vantande_detalj | — | Oförändrad | ✅ Live |
| 04 | verif_jamforelse | 1a | **Ändrad** — +sjukhusnamn i alla block | ✅ Klar |
| 04 | dq_rapport_data | 1b | **Ändrad** — DQ6 per sjukhus | ✅ Klar |
| 05 | ref_fhir_system | 2 | Oförändrad | ✅ Klar, ej skickat |
| 05 | fhir_measure_report | 2 | **3 fixar behövs** (kolumnnamn) | ⚠️ U10 |
...
| 7 | FHIR JSON | FHIR-tabell | 12 MeasureReports (214 KB) | KchdFhirSerializer (C#) ✅ Verifierad |
| 8 | Skicka till hubb | FHIR Bundle | Kvittens: 200 OK | REST API POST |

**Steg 7:** Python-referensen (`fhir_serializer.py`) och C#-versionen (`KchdFhirSerializer`) producerar identisk output. Verifierat 2026-03-28 i GitHub Codespace.
...
**Senaste push:** 2026-03-28 (C#-serialiserare verifierad + testverktyg)
**Väntar på push:** v2.4 VQL + fixat fhir_paket.vql + uppdaterad index
...
| Fil i repo | Senast ändrad | I repot? | v2.4 ändring |
|-----------|---------------|----------|-------------|
| README.md | 2026-03-28 | ✅ | — |
| .gitignore | 2026-03-26 | ✅ | — |
| **vql/katarakt_vantetid_v2.4.vql** | **2026-03-28** | **⬆️ NY** | **Komplett: alla 17 vyer i en fil** |
| vql/fhir_paket.vql | 2026-03-26 | ✅ | **⬆️ 3 fixar (U10)** |
| vql/komplettering_detaljvyer.vql | 2026-03-26 | ✅ | Ersätts av v2.4 |
| vql/verifieringspaket.vql | 2026-03-26 | ✅ | Ersätts av v2.4 |
| vql/kvalitetspaket.vql | 2026-03-26 | ✅ | Ersätts av v2.4 |
...
| docs/katarakt_index.md | 2026-03-28 | ⬆️ Uppdateras |
...
| csharp/KchdFhirSerializer/KchdFhirSerializer.csproj | 2026-03-28 | ✅ |
| csharp/KchdFhirSerializer/Program.cs | 2026-03-28 | ✅ |
| csharp/KchdFhirSerializer/DenodoReader.cs | 2026-03-28 | ✅ |
| csharp/KchdFhirSerializer/MeasureReportBuilder.cs | 2026-03-28 | ✅ |
| csharp/KchdFhirSerializer/FhirValidator.cs | 2026-03-28 | ✅ |
| csharp/KchdFhirSerializer/appsettings.json | 2026-03-26 | ✅ |
| csharp/test_csharp.sh | 2026-03-28 | ✅ |
| csharp/verify_fhir_structure.py | 2026-03-28 | ✅ |
...
Bygger C#, kör mot testdata, jämför mot Python-referens.
...
| Nr | Vad | Blockerar? | Status |
|----|-----|-----------|--------|
| 1 | **Applicera 3 fixar i fhir_paket.vql** (se FHIR_PAKET_FIXAR.md / U10) | Nej — dokumenterat | 📋 |
| 2 | **Pusha till GitHub:** katarakt_vantetid_v2.4.vql + fixad fhir_paket.vql + index | Nej | 📋 |
| 3 | Skicka mail till VGR: utomläningstest + patientresa-bugg | Nej — mail klart | 📧 Redo |
| 4 | Skicka mail till VGR: fråga om .NET 8 runtime + SSIS-version | Nej — mail redo | 📧 Redo |
| 5 | Vänta på VGR:s utomläningstest (AND LEFT(lkf,2) = '14') | Väntar på svar | ⏳ |
| 6 | Om VGR saknar .NET 8 → publicera C# som self-contained .exe | Väntar på svar | ⏳ |
| 7 | Leverera fas 2+3 som ett paket (VQL + C# + instruktioner) | Väntar på svar om .NET 8 | ⏳ |
| 8 | Fas 4: radnivå-FHIR (Procedure/ServiceRequest) | Väntar på fas 2+3 i produktion | 📋 |
| 9 | Expandera till fler regioner eller vårdutbud | Framtida | 📋 |
